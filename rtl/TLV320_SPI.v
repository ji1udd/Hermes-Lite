...
// 24 July  2016 - Revised for Hermes-Lite.
//                 MCLK, BCLK and SPI-CLK are generated by 
//		   ad9866clkX1(73.728MHz) posedge.
// (C) JI1UDD 2016
...
module TLV320_SPI (clk, rst, MCLKrise, nCS, MOSI, SSCK, boost, line, line_in_gain);

input  wire clk;
input  wire rst;			// active "H"
input  wire MCLKrise;
output reg  nCS;
output reg  MOSI;
output reg  SSCK;
...
localparam WaitTime = 200*12288 ;  // 200mS @CMCLK= 12.288Mhz
...
reg         current_boost ;
reg         current_line ;
reg   [4:0] current_line_in_gain ;
wire	    rstb = ~rst ;

always @ (posedge clk)				// add sync-FF
  if (MCLKrise) begin
    current_boost  <= boost ;
    current_line   <= line ;
    current_line_in_gain <= line_in_gain ;
  end
...
begin
  ...
  4'd2: TLV_data = current_line ? 16'h0810 : (16'h0814 + current_boost);	// D/A on
  4'd3: TLV_data = 16'h0C00;
  4'd4: TLV_data = 16'h0E02;
  4'd5: TLV_data = 16'h1000;
  4'd6: TLV_data = 16'h0A00;
  4'd7: TLV_data = {11'b0, current_line_in_gain};	// set line in gain
...
always @ (posedge clk or negedge rstb)
  if (!rstb) begin
    ...
    prev_boost <= 1'b0;
    prev_line  <= 1'b0;
    prev_line_in_gain <= 5'b0;

  end else if (MCLKrise) begin			// sync with MCLK posedge
    if (tlv_timeout != WaitTime)	      // wait for 200mS timeout
    ...
    case (TLV)
      ...
      4'd4:
        begin
          ...
          prev_boost <= current_boost; 	   // save the current boost setting 
          prev_line <= current_line;	   	// save the current line in setting
	  prev_line_in_gain <= current_line_in_gain; // save the current line-in gain setting
        end
      ...
      4'd5:
	    ...
	    nCS <= 1'b1;        				// set CS high               
            if (current_boost != prev_boost ||	// has boost or line in or line-in gain changed?
                current_line != prev_line ||
                current_line_in_gain != prev_line_in_gain) begin  
              load <= 0;
              TLV <= 4'd0;
            end else
